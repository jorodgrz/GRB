...
factor $\sim 2.6$ above the NSBH rate and exceeds it in 14 of the 20 models:
\compas\ generically overpredicts the NSBH rate. Third, the engine, not the
duration, sets the kilonova: against the \citet{rastinejad2024} sample,
long-duration bursts match the black-hole-disk prediction to $0.1$ dex while
short-duration bursts lie three orders of magnitude away, near the HMNS
expectation. Fourth, across the grid the local rate varies by two orders of
magnitude but the class ordering does not: the long-duration majority is the
robust prediction, while the exact fractions depend mainly on the dense-matter
equation of state. All GRB rates are upper limits, since they assume every
above-threshold system launches a jet.
\end{abstract}
...
population-synthesis grid every BHNS rate exceeds the tightened
NSBH region (per-model rates in
Section~\ref{sec:grid}; Figure~\ref{fig:frac20}). Independent
...
discriminating power rests on the assumed scaling.
...
Section~\ref{sec:lvk} is therefore generic but not universal. Model~J has $M_{\rm NS,max} = 2.0\,\Msun < \Mtov = 2.2\,\Msun$, so
...
\citep[shaded]{abac2026}. The weighted-Poisson sampling error is sub-percent
and the MSSFR-resolution systematic ($\sim \pm 30$ percent) is common to all
...
distribution. The HMNS class is present for every (model, EOS) pair, confirming
that the engine ordering survives the joint variation of binary-evolution and
dense-matter assumptions.
...
ordered family-major) and EOS (columns APR4, SFHo, LS220, DD2; constants from
\citealt{bauswein2013, bauswein2020} and \citealt{read2009}). Each EOS fixes
$\Mtov$ and the linked $\Mthresh = k\,\Mtov$. Color follows the perceptually
...
overprediction by the \compas\ BHNS physics, sharpest for the highest maximum NS
...
O3 sensitivity into the same cosmic integration (`detected_rate`), the model
...
itself: under the fiducial and the two softer-to-intermediate tabulations (APR4,
SFHo, LS220) the sbGRB-plus-blue class stays a minority (1 to 28 percent across
...
The sbGRB-minority ordering is thus robust to binary physics but not to the EOS:
a sufficiently stiff EOS reverses it. This is consistent with the overall
...
therefore generically overpredicts the GW-measured NSBH rate, most sharply at
the highest $M_{\rm NS,max}$.
...
$442\,\Gpcyr$) while the class ordering is preserved: the lbGRB-producing
classes hold at least 72 percent of the BNS population in every binary-physics
variation and the sbGRB-plus-blue class stays a minority. This ordering, not
the precise fractions, is the robust prediction. The detailed fractions depend
on the dense-matter equation of state, which moves the HMNS-class fraction from
27 to 5 percent at fixed binary physics through the linked
$(\Mtov, \Mthresh, M_{\rm HMNS})$ boundaries (Figure~\ref{fig:eosmatrix}), and
on the neutron-star mass remap that sets the BNS component-mass distribution
(Section~\ref{sec:remap}). The equation of state, not the binary-evolution
prescription, dominates the class-fraction uncertainty, and BH spin is the
dominant within-model source of BHNS rate uncertainty.
\end{enumerate}
