...
long-duration bursts dominate: 76\% of binary neutron star (BNS) mergers
produce one (27\% from an HMNS, 2\% from a disk, 47\%
faint), against a 24\% short-duration minority with blue kilonovae. The
black hole neutron star (BHNS) population is instead dominated by silent
mergers: 76\% swallow the neutron star and launch no burst at all. Second, the BNS local rate ($55\,\Gpcyr$) falls well inside the LVK
GWTC-5.0 90\% credible region, while the BHNS rate ($84.5\,\Gpcyr$) sits
...
the appendix to bound the two systematics that most strongly affect the remnant
...
\citet{broekgaarden2021} grid (Appendix~\ref{app:grid}). Methods are collected in Section~\ref{sec:methods}, results
...
population-synthesis robustness scan of Appendix~\ref{app:grid}. Two
...
all 20 variations enter the Appendix~\ref{app:grid} sweeps.}
...
comparatively stable (Appendix~\ref{app:grid}). The BHNS panel shows the
...
Appendix~\ref{app:grid}; Figure~\ref{fig:frac20}). Independent
...
the hatched all-BNS bar and the four \citet{gottlieb2024} class subsets that
sum to it.
\textit{Right:} BHNS at $\aBH = 0.5$, with the hatched all-BHNS bar and the two
GRB-class subsets. Both axes are intrinsic source-frame rates, so no beaming or
...
dominant model uncertainty on the local rate is the MSSFR-resolution systematic
(the TNG50-1 to TNG300-1 spread of Section~\ref{sec:classrates}) for BNS and
the black-hole-spin envelope over $\aBH \in [0,0.9]$ (Section~\ref{sec:spin})
for the BHNS GRB classes; the all-BHNS bar is spin-independent. Even across the
spin envelope the GRB-producing BHNS classes remain a small fraction of the
all-BHNS total, so the BHNS rate that overshoots the NSBH region is carried by
the NS-swallowing no-GRB systems rather than by the kilonova-bearing
population.
...
(Figure~\ref{fig:offsetkicks}, Appendix~\ref{app:grid}).
...
The robustness study (Appendix~\ref{app:grid}) shows the qualitative class
...
\citet{broekgaarden2021} grid of Appendix~\ref{app:grid}
...
every other grid variation: the \compas\ BHNS population
...
(Appendix~\ref{app:grid}); the equation of state, not the binary-evolution
...
\FloatBarrier
%% ======================================================================
\section{The population-synthesis grid} \label{app:grid}
%% ======================================================================
...
against the GWTC-5.0 regions, showing the two-orders-of-magnitude rate spread
against the near-invariant class composition discussed in
Section~\ref{sec:systematics}.
The BNS rate
stays within the GWTC-5.0 BNS region $[5.1, 154.7]\,\Gpcyr$ for the fiducial
and its neighbours: $55.0$ (A), $29.3$ (F, $\alpha_{\rm CE} = 0.5$), $106.0$
(G, $\alpha_{\rm CE} = 2.0$), $53.7$ (J), and $56.9\,\Gpcyr$ (K). Every BHNS
variation, in contrast, exceeds the tightened NSBH region
$[6.7, 32.8]\,\Gpcyr$, deepening with increasing maximum NS mass; the
overprediction noted in Section~\ref{sec:lvk} therefore applies across the
whole grid. Model~J has $M_{\rm NS,max} = 2.0\,\Msun < \Mtov = 2.2\,\Msun$, so
...
Figures~\ref{fig:alphace5} to \ref{fig:snengine} isolate the individual
assumptions that do move the fractions: the five-point common-envelope
sequence, the mass-transfer-efficiency sweep, and the delayed-versus-rapid
supernova engine.
...
\caption{Delayed (Model A) versus rapid (Model L, paper I Model~I) supernova
remnant engine.
...
The remaining panels carry the per-model diagnostics across the full grid.
Figure~\ref{fig:delaycdfmodel} shows the weighted BNS delay-time CDFs, faceted
by physics family with the fiducial Model~A overlaid in every panel. The
common-envelope efficiency shifts the distribution, with low-efficiency
Model~F at much longer delays \citep{vanson2022,broekgaarden2022}, whereas the
maximum-NS-mass and remnant-engine variations leave it nearly unchanged, as
expected for knobs that do not alter orbital separations.

\begin{figure*}[tbp]
\epsscale{1.15}
\plotone{delay_time_cdf_by_model_20models.pdf}
\caption{Weighted BNS delay-time CDFs ($t_{\rm delay} = t_{\rm form} + t_c$, in
Myr) for all 20 \citet{broekgaarden2021} variations, faceted by binary-physics
family with the fiducial Model~A (black) overlaid in every panel. The
common-envelope-efficiency family spreads the distribution most strongly; the
maximum-NS-mass and remnant-engine families leave the curve nearly unchanged.
A single 20-line overlay would be unreadable, so the per-family faceting keeps
the comparison legible. \compas, STROOPWAFEL-weighted.
